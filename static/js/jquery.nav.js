				...
				/* Filter any links out of the nav */		
					if(self.config.filter !== '') {		
							self.$nav = self.$nav.filter(self.config.filter);			}			
										...
										self.$nav.on('click.onePageNav', $.proxy(self.handleClick, self));			/* Get the section positions */		
											self.getPositions();				
																	...
																	self.$win.on('resize.onePageNav', $.proxy(self.getPositions, self));	
																			return this;		},	
																						adjustNav: function(self, $parent) {		
																							self.$elem.find('.' + self.config.currentClass).removeClass(self.config.currentClass);			$parent.addClass(self.config.currentClass);		},				bindInterval: function() {			var self = this;			var docHeight;				
